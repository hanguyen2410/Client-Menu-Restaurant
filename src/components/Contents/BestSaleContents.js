...
import BestSaleService from '../../Services/bestSaleService';
import FoodsDetail from './../FoodsDetail/FoodsDetail';

function BestSalesContent(props) {
  ...
  useEffect(() => {
    try {
      async function getData() {
        let productRes = await BestSaleService.getBestSaleProducts();
        setState({
          ...state,
          bestSaleProducts:
  ...
  }, [dragDown, dragMove, isDrag, bestSaleProducts]);
  ...
  return (
    <FoodsRowContainer draggable="false" id="TopSale">
      ...
      <FoodsSlider
        ...
        style={
          bestSaleProducts.length > 0 && bestSaleProducts
            ? {
              gridTemplateColumns: `repeat(${bestSaleProducts.length},
                            ${
                              windowWidth > 1600? '16%' 
                              :
                              windowWidth > 1400 ? '18%'
      ...
      >
        {console.log(typeof bestSaleProducts)}
        {bestSaleProducts.length > 0 &&
          bestSaleProducts.map((bestSaleProduct) => (
            <div
              key={bestSaleProduct.productId}
              ref={foodRef}
              draggable="false"
                ...
                <div className="flex items-center justify-between gap-8 ">
                  <p className="text-base text-headingColor font-semibold">
                    ...
                    {new Intl.NumberFormat('vi-VN', {
                      style: 'currency',
                      currency: 'VND',
                    }).format(bestSaleProduct.price)}
                  </p>
                </div>
                <p className="mt-1 text-sm text-gray-500">
                  {bestSaleProduct.unitTitle}
                </p>
              </div>
            </div>
          ))}
...
export default BestSalesContent;
